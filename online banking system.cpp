...
#include<windows.h>  //for adding colours to the text
#include<stdio.h> 
...
class Account : public user
{
	...
	void fd(int);               //function to create fixed deposit
	void viewFD(int);//       function to view all fixed deposit 
...
{	
	...
	if(flag == 0 )
	{
		...
		cout<<"\n User name and Passcode Not matched !"<<endl;
			cout<<string(26, '*');
...
void Account :: createAccount()
{
	...
	cin >>fname;
	cin.clear(); // to clear the wrong inputs 
	...
	cout << "\n\tEnter Date of Birth :";
	cin >>dob;
	cin.ignore();//skip charaters in input 
...
void Account :: deduct(int user_no,int amt)
{
	...
	if(user_no == accountNumber)
	{
			...
			else
			{
				...
				cout<<"\nYour transaction cannot be completed !!!!! Sorry your  Balance is Zero or Less than withdrawal amount\n! ";
				file_temp << fname <<" "<<lname<<" "<< phone<<" "<<dob<<" "<<type
				<<" "<<accountNumber<<" "<<passcode<<" "<<balance<<endl;
			}
	}
...
//	cout<<"\n\tAccount Number : "<<accountNumber<<endl;
	...
	cout<<"\n\n";
	cout<<"Total  Balance :"<<pbalance<<"/."<<endl;
	tr_file_read.close();
	
}

/*End mini statement function */
/*Start money transfer function */
void Account::fund_tr(int user_no)
{
	int user2_no ,amt =0,flag = 0;
	char ans;
	system("Color 0A");
	cout <<"\nEnter to Transfer Account Number: ";
	cin >> user2_no;
...
void user_menu(int user_sesstion)
{
   ...
   tt = localtime(&t);
	...
	cout<<"\n\t\t\t\t======================\n";
		...
		cout<<"\t\t\t\t    ::MAIN MENU::\n";
		cout<<"\n\t\t\t\t WELCOME YOUR ACCOUNT NUMBER :"<<user_sesstion<<endl;
		cout<<"\n\t\t\t\t1. VIEW YOUR ACCOUNT";
		cout<<"\n\t\t\t\t2. UPDATE DETAILS";
		cout<<"\n\t\t\t\t3. MONEY TRANSFER";
		cout<<"\n\t\t\t\t4. WITHDRAWAL AMOUNT";
		cout<<"\n\t\t\t\t5. DEPOSIT MONEY";
		cout<<"\n\t\t\t\t6. TRANSACTION HISTROY";
		cout<<"\n\t\t\t\t7. CLOSE ACCOUNT";
